...
void Sampling::sample_superEllipse(const double a1, const double a2, const double a4, const double e, const int N, pcl::PointCloud<PointT>::Ptr &cloud)
{
  ...
  do
  {
    ...
    {
      PointT p;
      p.x = a4 + (a1 * pow(fabs(cos(theta)), e));
  ...
  while(alpha>0 && numIter<maxIter)
  {
    ...
    numIter++;
    PointT p;
    p.x = a4+(a1 * pow(fabs(sin(alpha)),e));
...
void Sampling::sample_pilu_fisher()
{
  ...
  int N = 300;
  sample_superEllipse(1, params_.a3, params_.a4, params_.e1, N, cloud_s1);
  sample_superEllipse(params_.a1, params_.a2, 0.0, params_.e2, N, cloud_s2);
  PointT p1, p2;
